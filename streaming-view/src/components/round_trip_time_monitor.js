...
const DEFAULT_ROUND_TRIP_TIME_SERVER = window.location.protocol + "//" + window.location.hostname + ":" +
    window.location.port + window.location.pathname.replace(/\/$/, '') + "/rtt-websocket";
...
class RoundTripTimeMonitor extends Component {
    ...
    componentDidMount() {
        ...
        const socket = io(uri.protocol + "//" + uri.host, {path: (uri.pathname !== '/' ? uri.pathname : '') + "/socket.io"});
