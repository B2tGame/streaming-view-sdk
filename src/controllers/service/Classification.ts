import Metric, { MetricEventHandler } from './Metric';
...
export type ClassificationReport = {
  ...
  classification: LegacyClassificationType;
  // Classifications ordered by precedence
  classificationReport: ClassificationType[];
  duration: number;
...
type ClassificationType =
  | 'stream-not-resumed' // (possible user error)
  | 'unsupported-browser' // (error)
  | 'missing-iframe-stddev' // (error)
  | 'no-slow-motion-detected' // (good)
  | 'consistent-slow-motion-detected' // (bad)
  | 'slow-beginning-detected' // (bad)
  | 'slow-start-detected' // (acceptable)
  | 'no-classification-detected'; // (error)
/**
 *  There are three "types" of classification,
 *  good, bad and error. They are put in an
 *  ordered by precedence like such:
...
type LegacyClassificationType =
  ...
  | 'consistent-slow-motion-detected'
  | 'slow-beginning-detected'
  | 'slow-start-detected'
  | 'no-classification-detected'
  | 'unsupported-device';

export default class Classification {
  numberReportsCreated: number = 0;
  metricPeriodEndHandlers: Array<MetricEventHandler> = [];
  classificationRecordingStarted: boolean = false;

  browser: UserAgentParser;
  metricsFramesDecodedPerSecond: Metric;
  isClassificationReportCreated: boolean = false;
  metricsInterFrameDelayStandardDeviation: Metric;
  framesDecodedPerSecondHistogram: FramePerSecondHistogram;

  constructor(public streamingViewId: string, public logger: Logger) {
  ...
  injectMeasurement(framesDecodedPerSecond: number, interFrameDelayStandardDeviationInMs: number) {
  ...
  registerMetricEventListener(callback: (evt: CustomEvent<any>) => void) {
    // We only care about proxying one of either the two metrics that are registered, so it's a bit of a hack.
    const triggeredMetrics: { [key: string]: boolean } = {};

    const handleMetricPeriodEnd: MetricEventHandler = (evt) => {
  ...
  createClassificationReport(reportTrigger = 'unknown'): ClassificationReport {
    ...
    const createClassification = (): ClassificationType[] => {
      const classificationReport: ClassificationType[] = [];
