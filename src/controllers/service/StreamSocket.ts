...
// @ts-ignore a weird module
import io, { Socket } from 'socket.io-client';
import pako from 'pako';
import * as log from '../../measurements/Logger';
import { USER_EVENT_REPORT_PAYLOAD } from '../../measurements/StreamingEventNames.js';
...
export default class StreamSocket {
   ...
   */
  constructor(public edgeNodeId: string, streamEndpoint: string, public userId: string, internalSession: boolean) {
    const endpoint = new URL(streamEndpoint);
    this.socket = io(`${endpoint.protocol}//${endpoint.host}`, {
      path: `${endpoint.pathname}/emulator-commands/socket.io`,
      query: {
        userId,
        internal: internalSession ? '1' : '0',
      },
    });
    this.reportCache = [];
    this.reportIntervalId = window.setInterval(this.emitReports, StreamSocket.WEBSOCKET_EMIT_REPORTS_INTERVAL);
    ...
    this.pingIntervalId = window.setInterval(() => {
      ...
      this.socket?.volatile.emit('get-server-time', (serverTimestamp: number) => {
        const end = Date.now();
    ...
    this.socket.on('message', (data) => {
      ...
      if (message.name === 'emulator-configuration') {
        StreamingEvent.edgeNode(edgeNodeId).emit(StreamingEvent.EMULATOR_CONFIGURATION, message.configuration as EmulatorConfiguration);
        ...
        switch (message.event) {
          ...
          case 'terminated': {
            ...
            window.window.clearInterval(this.pingIntervalId);
            break;
          }
  ...
  onReportMeasurement = (payload: StreamingEvent.REPORT_MEASUREMENT_PAYLOAD[0]) => {
  ...
  emitReports = (isLast = false) => {
    if (isLast) {
      window.window.clearInterval(this.reportIntervalId);
    }
