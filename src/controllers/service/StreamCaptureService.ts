...
export default class StreamCaptureService {
  ...
  constructor(
    ...
    public canvasTouch: RefObject<HTMLCanvasElement>
  ) {}
   ...
   * @returns {boolean}
   */
  detectTouch = (x: number, y: number, emulatorWidth: number, emulatorHeight: number) => {
   ...
   */
  edgeDetection = function (ctx: CanvasRenderingContext2D, canvasWidth: number, pixelData: Uint8ClampedArray) {
   ...
   */
  plotPoint = function (ctx: CanvasRenderingContext2D, x: number, y: number) {
    ctx.beginPath();
   ...
   * @returns {string}
   */
  captureScreenshot = (emulatorWidth: number, emulatorHeight: number) => {
   ...
   * @return {boolean}
   */
  isSameColor(pixel1: PixelData, pixel2: PixelData) {
   ...
   * @returns {{red: number, green: number, blue: number}}
   */
  getPixel(image: ImageData, offset: number): PixelData {
    return {
  ...
  rgbToHex(pixel: PixelData): string {
