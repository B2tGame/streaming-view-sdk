...
export default class BlackScreenDetector {
  ...
  workingStreamLatestTimestamp = Date.now();
  notVisibleHoldOffPeriod = 0;
  recentEvents: Array<{ event: any; payload: any }> = [];
   ...
   */
  constructor(public edgeNodeId: string, public streamingViewId: string) {
   ...
   */
  onStreamVideoScreenshot = (event: StreamingEvent.STREAM_VIDEO_SCREENSHOT_PAYLOAD[0]) => {
    if (event.hasVideo) {
  ...
  browserTabIsVisible() {
    ...
    if (typeof document.hidden !== 'undefined') {
      ...
      // @ts-ignore MS prefix is required for IE11
    } else if (typeof document.msHidden !== 'undefined') {
      // @ts-ignore MS prefix is required for IE11
      documentHidden = document.mHsidden;
      // @ts-ignore WebKit prefix is required for older browsers
  ...
  destroy() {
    if (this.monitorIntervalId) {
      window.window.clearInterval(this.monitorIntervalId);
    }
