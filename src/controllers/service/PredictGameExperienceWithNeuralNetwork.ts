...
export interface NeuralNetworkData {
  ...
  leakyReluAlpha?: string;
}

export type NeuralLayer = {
  [layer: string]: NeuralData;
};
 ...
 */
export default class PredictGameExperienceWithNeuralNetwork extends PredictGameExperienceBase {
  network: Function;
  ...
  /**
   *
   *  @param neuralNetwork Brain.js network to use.
   */
  constructor(neuralNetwork: NeuralNetworkData) {
    super();

    this.network = this.toFunction(neuralNetwork, (key) => key === 'score');
    this.resultBufferSize = neuralNetwork.outputSampling;
    this.inputBufferSize = neuralNetwork.samplingSize;
    this.inputBuffer = [];
    this.resultBuffer = [];
  }
   ...
   * @param filter Filter function for select only a subset of the network to be calculated.
   */
  toFunction(json: NeuralNetworkData, filter: (key: string) => boolean) {
    let needsVar = false;

    function nodeHandle(layers: NeuralLayer[], layerNumber: number, nodeKey: string) {
      if (layerNumber === 0) {
      ...
      for (let w in node.weights) {
        if (node.weights[w] < 0) {
          result.push(''.concat(node.weights[w].toString(), '*').concat(nodeHandle(layers, layerNumber - 1, w)));
        } else {
          result.push('+'.concat(node.weights[w].toString(), '*').concat(nodeHandle(layers, layerNumber - 1, w)));
        }
      }
      ...
      switch (json.activation) {
        ...
        default: {
          throw new Error(
            'Unknown activation '.concat(json.activation, ". Available activations are: 'sigmoid', 'relu', 'leaky-relu', 'tanh'")
          );
        }
      }
    }
