 ...
 */
const retryWithTimeout = <Res>(timeout: number, httpRequest: () => Promise<Res>): Promise<Res> => {
  let timeoutElapsed = false;
  window.setTimeout(() => (timeoutElapsed = true), timeout);

  const repeat = (resolve: (value: Res | PromiseLike<Res>) => void, reject: (reason?: any) => void) => {
    // resolve on success
    httpRequest().then(resolve, (err) => {
      ...
      // retry on error
      return window.setTimeout(() => repeat(resolve, reject), 10);
    });
  };
...
type ServiceCoordinatorStatusResponse = { status: string };
type EdgeNodeStatus = { state: string; endpoint?: string; isReadyToAcceptConnection?: boolean };
type ResolvedEdgeNodeStatus = EdgeNodeStatus & { endpoint: string };
...
class StreamingController {
  ...
  constructor(options: StreamingControllerOptions) {
    ...
    try {
      ...
      throw new Error(`StreamingController: invalid apiEndpoint, got "${options.apiEndpoint}" as input`);
    }

    this.apiEndpoint = options.apiEndpoint;
    this.edgeNodeId = options.edgeNodeId || undefined;
    this.internalSession = options.internalSession || false;
  }
  ...
  /**
   * Terminate the instance (optionally provide a termination reason for logging purposes)
   * @param {string} reason
   */
  terminate(reason: string) {
    ...
    return this.getStreamEndpoint().then((streamEndpoint) =>
      axios.post<ServiceCoordinatorStatusResponse>(`${streamEndpoint}/emulator-commands/terminate`, body)
    );
  }
  ...
  softTerminate() {
    return this.getStreamEndpoint().then((streamEndpoint) =>
      axios.get<ServiceCoordinatorStatusResponse>(`${streamEndpoint}/emulator-commands/soft-terminate`)
    );
  }
  ...
  diskSnapshot() {
    return this.getStreamEndpoint()
      .then((streamEndpoint) => {
        return axios.get<string>(`${streamEndpoint}/emulator-commands/disk-snapshot`);
      })
      .then((resp) => {
        if (resp.data.toString().startsWith('FAIL')) {
          throw new Error(resp.data.toString());
  ...
  save() {
    return this.getStreamEndpoint()
      .then((streamEndpoint) => {
        return axios.get<string>(`${streamEndpoint}/emulator-commands/save`);
      })
      .then((resp) => {
        if (resp.data.toString().startsWith('FAIL')) {
          throw new Error(resp.data.toString());
  ...
  pause() {
    return Promise.all([this.getEdgeNodeId(), this.getStreamEndpoint()]).then(([edgeNodeId, streamEndpoint]) => {
      ...
      return axios.get<ServiceCoordinatorStatusResponse>(`${streamEndpoint}/emulator-commands/pause`);
    });
  }
  ...
  resetMoment() {
    return Promise.all([this.getEdgeNodeId(), this.getStreamEndpoint()]).then(([edgeNodeId, streamEndpoint]) => {
      ...
      return axios.get<ServiceCoordinatorStatusResponse>(`${streamEndpoint}/emulator-commands/reset`).then((response) => {
  ...
  resume() {
    return Promise.all([this.getEdgeNodeId(), this.getStreamEndpoint()]).then(([edgeNodeId, streamEndpoint]) => {
      ...
      return axios.get<ServiceCoordinatorStatusResponse>(`${streamEndpoint}/emulator-commands/resume`);
    });
  }
   ...
   * @param stillWaiting Given the status data, return true if waitWhile should wait longer.
   * @param timeout Max duration the waitFor should wait before reject with an timeout exception.
   */
  async waitWhile(
    stillWaiting: (data: EdgeNodeStatus) => boolean,
    timeout = StreamingController.DEFAULT_TIMEOUT
  ): Promise<ResolvedEdgeNodeStatus> {
...
const factory = async (options: StreamingControllerOptions) => {
