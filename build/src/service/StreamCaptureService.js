...
class StreamCaptureService {
  ...
  constructor(edgeNodeId, video, canvas, canvasTouch) {
    this.detectTouch = (x, y, emulatorWidth, emulatorHeight) => {
    ...
    this.captureScreenshot = (emulatorWidth, emulatorHeight) => {
      ...
      if (this.canvas.current && this.video.current) {
        ...
        const pixels = []; // Offset is used to avoid corners of the stream video to lower number of faulty video pixels
        ...
        _StreamingEvent.default.edgeNode(this.edgeNodeId).emit(_StreamingEvent.default.STREAM_VIDEO_SCREENSHOT, {
          hasVideo: hasVideo,
